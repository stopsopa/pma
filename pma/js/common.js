...
$(function () {
    ...
    $(window).hashchange(function() {
        var fields = $('.pmd_tab tbody .nowrap');
        fields.each(function () {
            var t = $(this);
            var te = $.trim(t.text());
            log(te.length)
            if (te.length > 29) {
                var text = $.trim(t.html().replace(/\s+/g,' '));
                var parts = text.match(/(<img[^>]*>)(.*)/);
                if (parts.length == 3) {
                    var img = parts[1];
                    var text1 = parts[2];
                    var text2 = text1.substring(0, 24)+" ...";
                    var html = "<span title=\""+text1+"\">"+img+text2+"</span>";
                    t.html(html);
                }
                else {
                    log('nie da się rozbić regexpem /(<img[^>]*>)(.*)/ : "'+text+'"')
                }
            }
        });
        $('#id_scroll_tab :checkbox:first').click();
    }).hashchange();
